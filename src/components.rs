...
impl Emulator {
    ...
    pub fn get_rpl (&self, addr: u8) -> u8 { // addr HAS to be between 0 and 7, nothing more.
        self.rpl[addr as usize]
    }
    pub fn set_rpl (&mut self, addr: u8, val: u8) { // addr is same as get_rpl
    ...
    pub fn draw_hi (&mut self, x: usize, y: usize) -> bool { // returns whether this draw call intersected
        if x >= 128 || y >= 64 {
            return false
        }
        if self.display[x][y] {
    ...
    pub fn draw_lo (&mut self, x: usize, y: usize) -> bool { // returns same as draw_hi, just doubles coordinates
        if x >= 64 || y >= 32 {
            return false
        }
